...
import 'dart:convert';
import 'missionDetailsPage.dart';


class HistoriqueMission extends StatefulWidget {
  const HistoriqueMission({Key? key}) : super(key: key);

  @override
  _HistoriqueMissionState createState() => _HistoriqueMissionState();
}

class _HistoriqueMissionState extends State<HistoriqueMission> {
  DateTime? startDate;
  DateTime? endDate;
  bool showMenu = false;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => MissionDetailsPage(mission : mission, missiona: missiona),
          ),
        );
      } else {
